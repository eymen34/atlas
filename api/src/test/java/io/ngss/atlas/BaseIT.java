...
public abstract class BaseIT {
   ...
   *
   * <pre>
   * outbox → ticket_links → attachments → notifications → ticket_mentions → ticket_watchers
   ...
   * </pre>
   *
   * <p>{@code outbox} (T-029) has no FK dependents and references no other table, so it
   * deletes first as a free-standing infra table.
   *
